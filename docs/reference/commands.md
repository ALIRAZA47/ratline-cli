...
OTHER
  schema       Print the whole command surface as JSON, for tools and agents
  mcp          Serve ratline to an AI agent over MCP on stdin and stdout
...
```
Every command, every flag with its type and whether it is required, the exit
...
Examples:
  ratline schema | jq '.commands[] | select(.name=="site")'
...
```
Speaks the Model Context Protocol over stdio, so an agent can read the state of
...
Flags:
      --allow-mutations   Also expose the tools that change this server (site deploy, site restart)
  -h, --help              help for mcp
