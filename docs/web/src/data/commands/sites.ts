...
ratline cert issue example.com --email admin@example.com`,
  ...
  ratline site restart "$d"
    ...
    {
      id: 'site-exec',
      name: 'ratline site exec',
      args: '<domain> -- <command>',
      status: 'built',
      summary: 'Run one command as the site’s tenant, with the site’s environment and runtime.',
      description: [
        'The conditions are the build command’s: the tenant’s identity, the application directory, the site’s .env, and the runtime the site is pinned to first on PATH. So `npm` is the npm belonging to this site’s Node version and `python` is the one in its venv, on a server with neither installed system-wide — which is what makes a seed, a migration or a one-off management command runnable at all. The managed runtimes under /opt/ratline/runtimes are deliberately not on anybody’s PATH, because they are per-site and because systemd never reads a login shell.',
        'Everything after `--` is an argv, not a command line: the first word is the program and the rest are its arguments, handed to execve untouched. A single quoted argument is split by the same parser that splits a build command. Nothing reaches a shell in either form.',
        'The program’s output is the command’s output, so it pipes; under --json it is captured into the envelope instead, because stdout has to stay one parseable object. Its exit code is reported but not adopted — ratline’s exit codes are a contract, so a failing program exits 4 and names the code it gave.',
      ],
      flags: [
        {
          name: '--cwd',
        ...
        },
        {
          ...
          arg: '<duration>',
          type: 'duration',
          default: 'runtimes.build_timeout',
          description: 'Give up after this long.',
          note: 'The command holds the server lock while it runs, so it is bounded rather than open-ended.',
        },
        {
          name: '--stdin',
          type: 'bool',
          default: 'false',
          description: 'Pipe this command’s standard input through to the program.',
          note: 'Without it the program gets /dev/null, so one that reads stdin ends instead of waiting for nobody.',
        },
      ],
      refuses: [
        'An argument that is a shell operator — |, ||, &&, &, >, >>, < or << — rather than passing it to the program as a word. A lone ; is allowed, because `find -exec … ;` passes one legitimately.',
        'A program name containing shell metacharacters, which means the whole line was typed as if a shell were going to read it.',
        'A program it cannot find, naming the directories it searched rather than letting execve report a path in /usr/bin nobody typed.',
        'A file that exists but has no execute bit, with the chmod that fixes it.',
      ],
      exits: [
        { code: 2, reason: 'The argv was empty, contained a shell operator, or carried a control character.' },
        { code: 3, reason: 'No such site, or the program was not found, is a directory, or is not executable.' },
        { code: 4, reason: 'The program ran and exited non-zero, or timed out. Its own exit code is in the message and in the --json envelope.' },
        { code: 5, reason: 'Locked.' },
      ],
      examples: [
        { title: 'A package.json script', lang: 'shell', code: 'ratline site exec app.example.com -- npm run bootstrap' },
        { title: 'A Django management command', lang: 'shell', code: 'ratline site exec api.example.com -- python manage.py migrate' },
        {
      ...
      seeAlso: [
        { label: 'Scheduled jobs', to: '/reference/site/cron' },
        { label: 'Deploy hooks', to: '/reference/site/hook' },
      ],
      keywords: ['npm run', 'run a script', 'one-off', 'seed', 'migrate', 'management command', 'shell'],
    },
    {
      id: 'site-logs',
    ...
    },
    {
      ...
      args: '<domain>',
      status: 'built',
      summary: 'Manage the site’s .env, which ratline-shell exec loads as the site user when the service starts.',
      description: [
        '.env lives at /home/<user>/<domain>/.env, mode 0600 owned by the site user. The unit carries no EnvironmentFile= — that would have PID 1 open a tenant-owned path as root, and a symlink put there would be a root read of whatever it pointed at. Instead ExecStart runs ratline-shell exec --env-file <site>/.env -- <program>, after User= has taken effect, so the file is opened by a process that is already the tenant, merged over the unit’s Environment= lines, and the program is exec’d in place. That is why the file can be 0600 and the app can still see it. It is never inside a directory nginx can serve, and nginx additionally denies dotfiles.',
      ...
      refuses: [
        'A value containing a newline: .env is KEY=VALUE lines in the format systemd’s EnvironmentFile= defines, which ratline-shell exec parses the same way, and it cannot represent a multi-line value. Store the payload in a file inside the site directory and point a variable at it.',
...
ratline site clone app.example.com staging.example.com --with-files --start` },
    ...
    },
    {
      ...
      description: [
        'A job runs on a schedule as the site’s tenant, in the site’s directory, with the site’s .env, sandbox, memory ceiling and PATH — so `npm` is the npm belonging to the Node version the site is pinned to, exactly as it is for a deploy or for `site exec`.',
      ...
      flags: [
        ...
        { name: '--command', arg: '<command>', type: 'string', required: true,
          description: 'What to run: a program on the site’s own PATH (`npm run nightly`) or an absolute path, with arguments.',
          note: 'The unit carries the site’s PATH — its venv, its node_modules/.bin and the interpreter it is pinned to — so a program named by word is the site’s own. systemd parses the line itself, so it is an argv rather than a shell line: a pipe or a redirection is refused, and a relative path is refused when the job is created rather than when the timer fires.' },
...
ratline site cron logs app.example.com nightly` },
    ...
    },
    {
      ...
      flags: [
        { name: '--command', arg: '<command>', type: 'string', required: true,
          description: 'What to run: a program on the site’s own PATH (`npm run worker`) or an absolute path, with arguments.' },
