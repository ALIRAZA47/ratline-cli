...
git-only: git`,
  ...
  },
  {
    key: 'runtimes',
    title: 'runtimes',
    blurb: 'Managed Node, Bun and Python versions.',
    settings: [
      {
      ...
      {
        key: 'runtimes.bun_default',
        value: '""',
        type: 'string',
        note: 'Empty until `ratline runtime install bun` has run. A bun site with no --bun follows it.',
      },
      { key: 'runtimes.node_mirror', value: 'https://nodejs.org/dist', type: 'url' },
      {
        key: 'runtimes.bun_mirror',
        value: 'https://github.com/oven-sh/bun/releases',
        type: 'url',
        note: 'The release host, not a directory index: the tag is appended, giving <bun_mirror>/download/bun-v<version>/bun-linux-<arch>.zip, verified against the SHASUMS256.txt published beside it.',
      },
  ...
  },
  {
    ...
    settings: [
      ...
      {
        key: 'databases.mysql.default_role',
        value: 'readWrite',
        type: 'string',
        note: 'Granted to a user created without --role for `db --engine mysql`. readWrite maps to SELECT, INSERT, UPDATE, DELETE on the user’s own database — enough for an application, without the right to create users or drop the schema.',
      },
      {
        key: 'databases.mysql.env_key',
        value: 'DATABASE_URL',
        type: 'string',
        note: 'The variable a MySQL connection string is written to in a site’s .env.',
      },
      {
        key: 'databases.mysql.timeout',
        value: '30s',
        type: 'duration',
        note: 'One mysql-client invocation, so a server that hangs behind an access list becomes an error naming the cause rather than a command that never returns.',
      },
      {
        key: 'databases.redis.default_role',
        value: 'readWrite',
        type: 'string',
        note: 'Granted to an ACL user created without --role for `db --engine redis`. readWrite maps to the +@read +@write command categories within the user’s keyspace, never @dangerous.',
      },
      {
        key: 'databases.redis.env_key',
        value: 'REDIS_URL',
        type: 'string',
        note: 'The variable a Redis connection string is written to in a site’s .env.',
      },
      {
        key: 'databases.redis.timeout',
        value: '30s',
        type: 'duration',
        note: 'One redis-cli invocation.',
      },
    ],
  },
  {
